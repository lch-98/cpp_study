/*
 * [Topic] HTTP Request + JSON Parsing (Weather API)
 * libcurl로 기상청 API를 호출하고 jsoncpp로 JSON을 파싱하는 예제.
 *
 * 사용 라이브러리:
 *   - libcurl  : HTTP GET 요청
 *   - jsoncpp  : JSON 파싱
 ...
 * 동작 흐름:
 *   1. 현재 날짜/시간 생성
 *   2. 기상청 초단기실황 API URL 조립
 *   3. curl로 HTTP GET 요청 → 응답을 문자열로 수신
 *   4. JSON 파싱 → T1H(기온), PTY(강수형태) 추출
 *   5. 강수 코드를 한글 문자열로 변환 후 출력
 *
 * 사용 전 필수:
 *   - serviceKey 를 공공데이터포털(data.go.kr)에서 발급받은 키로 교체
 *   - CMakeLists.txt 에서 libcurl, jsoncpp 링크 설정 필요
 */
#include <iostream>
#include <string>
#include <sstream>
